...
public class ExcelChartUtil22 {
    private static SXSSFWorkbook wb = null;
    ...
    private boolean drawSheet0Map(SXSSFSheet sheet, STBarGrouping.Enum group, List<String> fldNameArr,
        ...
        ClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, 9, 8, 19, 25);
        // 创建一个chart对象
    ...
    private boolean drawSheet0Table(SXSSFSheet sheet, List<String> titleArr, List<String> fldNameArr,
        ...
        //System.out.println("row0=="+row0.getCell(1));
        // 填充数据
        for (int i = 0; i < dataList.size(); i++) {
            ...
            for (int j = 0; j < data.size(); j++) {
                ...
                if (j == 0) {
                    ...
                    rowi.createCell(j).setCellValue((Double) data.get("value" + (j + 1)));
                    // 设置数据样式
